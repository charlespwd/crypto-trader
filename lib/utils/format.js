...
function formatPerformances(performances, tickers) {
    var table = new Table({
        head: [
            'Pair',
            'Total spent',
    ...
    var sortByUSDValue = sortBy(function (pair) { return conversions_1.btcToUSD(pair[1].profit, tickers); });
    var transform = pipe(filter(function (x) { return startsWith('BTC', x.currencyPair); }), function (x) { return toPairs(x); }, sortByUSDValue);
